...
public interface CourseDao extends CrudRepository<Courses,Integer> {
    ...
    @Query(value = "SELECT `id`, `course_date`, `course_description`, `course_duration`, `course_title`, `course_venue` FROM `courses` WHERE `course_title` = :course_title",nativeQuery = true)
